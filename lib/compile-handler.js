...
export default {
  ...
  //params: javaPath, workingDir, filename, ext, output: called to log and clear output
  async start(javaPath, workingDir, filename, ext, output) {
    this.output = output;
    ...
    this.compileProc = spawn(javaPath + '/bin/' + 'javac', [fullName], { cwd: workingDir });
    ...
    try {
      ...
      this.execProc = spawn(javaPath + '/bin/' + 'java', [filename], { cwd: workingDir });
